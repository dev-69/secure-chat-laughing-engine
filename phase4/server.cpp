    ...
    while(true) {
        ...
        if(msg == "/quit") {
            ...
            if(space_pos != std::string::npos) {
                ...
                if (actual_msg.rfind("__E2E_INIT__", 0) == 0 || actual_msg.rfind("__E2E_ACK__", 0) == 0 ||
                    actual_msg.rfind("__E2E_MSG__", 0) == 0) { 

                    formatted = "__E2E_FROM__" + username + "__" + actual_msg;
                }
                else if (actual_msg.rfind("__E2E_ACK__", 0) == 0 || actual_msg.rfind("__E2E_MSG__", 0) == 0) {
                    formatted = actual_msg;
                }
                else {
                ...
                if(clients.find(target_user) != clients.end()) {
                    ...
                    sendAll(target_socket, enc_formatted.data(), enc_formatted.size());
                    if (actual_msg.rfind("__E2E_MSG__", 0) == 0) {
                        logToConsole("[CHAT-E2E] " + username + " -> " + target_user + " : " + actual_msg);
                    }
                    else {
